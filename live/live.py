...
from qvi import CurrencyConverter,calc_cov_matrix_annualized,intWithCommas,justify_str,\
                ...
                calc_expected_return_before_201703,calc_irr_mean_cov_after_20170309,\
...
# mean_irr_list = calc_expected_return_before_201703(config_common,datetime.now().date(),symbol_list,hist_bps_dict,hist_unadj_px_dict,hist_operincm_dict,hist_totasset_dict,hist_totliabps_dict,hist_costofdebt_dict,hist_stattaxrate_dict,hist_oper_eps_dict,hist_eps_dict,hist_roa_dict,0,True)
symbol_with_enough_data_list,irr_combined_mean_list,irr_combined_cov_list,irr_combined_ci_list = calc_irr_mean_cov_after_20170309(config_common,datetime.now().date(),symbol_list,hist_bps_dict,hist_unadj_px_dict,hist_totliabps_dict,hist_eps_dict,hist_roa_dict,10000,0,True)
if any(map(lambda x: x is None, [symbol_with_enough_data_list,irr_combined_mean_list,irr_combined_cov_list,irr_combined_ci_list])):
    print "Problem calculating IRR. Exiting..."
    sys.exit(0)

irr_combined_mean_dict = dict(zip(symbol_with_enough_data_list,irr_combined_mean_list))
...
#     cur_port_exp_rtn = float(np.asarray(mean_irr_list) * cur_pos_vec_T)
...
#     print cov_matrix
#
# str_expected_rtn_list = map(lambda x: str(round(x,4)*100)+'%', mean_irr_list)
...
# str_sharpe_list = map(lambda x: str(round(x[0]/x[1],3)), zip(mean_irr_list,annualized_adj_sd_list))
# # print
...
#
# from_tgt_rtn = min(mean_irr_list)
# to_tgt_rtn = max(mean_irr_list)
#
...
#     tmp_sol_list = markowitz_sharpe(symbol_list, mean_irr_list, cov_matrix, max_weight_list, float(config["general"]["min_expected_return"]), industry_groups_list, float(config["max_weight"]["industry"]), float(config["general"]["portfolio_change_inertia"]), float(config["general"]["hatred_for_small_size"]), current_weight_list)
...
#     tmp_sol_list = log_optimal_growth(symbol_list, mean_irr_list, cov_matrix, max_weight_list, industry_groups_list, float(config["max_weight"]["industry"]), float(config["general"]["portfolio_change_inertia"]), float(config["general"]["hatred_for_small_size"]), current_weight_list)
#     if tmp_sol_list is None:
#         print "Failed to find solution."
#         sys.exit(0)
...
# target_port_exp_rtn = float(np.asarray(mean_irr_list) * sol_vec_T)
...
# target_port_exp_dollar_rtn_list = map(lambda x: x[0]*x[1]*float(config["general"]["capital"]), zip(sol_list,mean_irr_list))
#
#
# ###################################################
# print
...
# ###################################################
# if len(current_pos_list) > 0:
#     current_port_exp_dollar_rtn_list = map(lambda s: (s,int(irr_combined_mean_dict[s] * current_mkt_val_dict[s])), current_mkt_val_dict.keys())
...
# sym_sol_list_with_diff = map(lambda x: (x, int(x[1] * float(config["general"]["capital"]) - current_mkt_val_dict.get(x[0],0)), irr_combined_mean_dict[x[0]]), sym_sol_list)
...
# columns.append(map(lambda x: "   ", sym_sol_list))
# columns.append(map(lambda x: justify_str(round(irr_combined_mean_dict[x[0]]*100,2),10), sym_sol_list))
# columns.append(map(lambda x: " %    ", sym_sol_list))
