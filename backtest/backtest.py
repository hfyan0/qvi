...
for dt in rebalance_date_list:
    ...
    from_tgt_rtn = min(irr_combined_mean_list)
    ...
    if (config["general"]["construction_method"] == "markowitz_max_kelly_f"):
        ...
        for i in range(N):
            ...
            if config["general"]["robust_optimization"].lower() == "true":
                tmp_sol_list = markowitz_robust(symbol_list, irr_combined_mean_list, cov_matrix, mu_p, max_weight_list, extract_sd_from_cov_matrix(cov_matrix))
            else:
                tmp_sol_list = markowitz(symbol_list, irr_combined_mean_list, cov_matrix, mu_p, max_weight_list, 0.0, industry_groups_list, float(config["max_weight"]["industry"]))
            ...
            frontier_port_exp_rtn = float(np.asarray(irr_combined_mean_list) * sol_vec_T)
    ...
    if config["general"]["hedging_type"].lower() == "beta":
        ...
        new_rtn_vec = ([sum(map(lambda x: x[0]*x[1], zip(irr_combined_mean_list,sol_list)))] + map(lambda hs: hedge_expected_rtn_dict[hs], hedging_symbol_list))
